...
This documents the JSON payloads each Gemini hook receives on stdin for every event type.
...
Fired when a Gemini session begins. **No stdin input** — the hook just runs and its stdout is shown to Gemini as context.
...
Fired when the Gemini session ends. **No stdin input** — the hook runs for cleanup and logging.
...
| 2 | Block (stderr shown to Gemini) | PreToolUse only |
