...
This is a complete Gemini agent architecture for game development. It
...
2. **Set up the engine** — Run `/setup-engine` (uses the brainstorm recommendation)
   - Configures GEMINI.md, detects knowledge gaps, populates reference docs
...
```
GEMINI.md                          -- Master config (read this first, ~60 lines)
.agents/
  settings.json                    -- Gemini hooks and project settings
  subagents/                       -- 48 subagent definitions (YAML frontmatter)
